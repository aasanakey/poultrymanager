...
class ExpensesController extends Controller
{
    ...
    public function getMonthlyExpense($model, $month, $year = null, $category = chicken)
    {
        ...
        switch ($model) {
            case 'Birds':
                $model = "\\App\\$model";
                $monthly_expense_collection = $model::selectRaw("sum(number * unit_price) as expense ")->where("farm_id", auth()->user()->farm_id)
                    ->where("bird_category", $category)->whereMonth("date", $month)->whereYear("date", $year)->pluck('expense');
                break;
            case 'Feed':
                $model = "\\App\\$model";
                $monthly_expense_collection = $model::selectRaw("sum(price) as expense")->where('farm_id', auth()->user()->farm_id)
            ...
            case 'Transaction':
                $model = "\\App\\$model";
                $monthly_expense_collection = $model::selectRaw("sum(amount) as expense")->where('farm_id', auth()->user()->farm_id)
                    ->where("farm_category", $category)->where('type','expense')->whereMonth("date", $month)->whereYear("date", $year)->pluck('expense');
                break;
        }
        $monthly_expense_collection = json_decode($monthly_expense_collection);
        $month_sales = null;
        if (!empty($monthly_expense_collection)) {
            foreach ($monthly_expense_collection as $sales) {
     ...
     * @param string $year
     * @param string $category
     * @return array $monthly_expense_data_array
     */
    ...
    public function getMonthlyExpenseData($model, $year = null, $category = null)
    {
        ...
        $max_no = !empty($monthly_expense_array) ? max($monthly_expense_array) : 0;
        $max = round(($max_no + 10 / 2) / 10) * 10;
        $monthly_expense_data_array = array(
            'months' => $month_name_array,
            'sales' => !empty($monthly_expense_array) ? $monthly_expense_array : [0],
            'max' => $max,
        );
        return $monthly_expense_data_array;
    }

    public function test(Request $request)
    {
        $models = ['Birds', 'Feed', 'Medicine','Transaction'];
        $sales_array = ['Bird' => [], 'Feed' => [], 'Medicine' => [],'Transaction'=>[]];
        // $sales_array = [];
        foreach ($models as $key => $model) {
            // $model, $month, $year = null, $category = null, $price_field
            $sales = $this->getMonthlyExpenseData($model, '2020', 'chicken');
            dump($sales);
    ...
    public function getSales(Request $request, $type = null)
    {
        ...
        foreach ($models as $key => $model) {
            $sales = $this->getMonthlyExpenseData($model, $year, $type);
            array_push($sales_array[$model], $sales);
            $final_max = max($final_max, $sales['max']);
        }
